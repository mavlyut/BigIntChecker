...
big_integer::big_integer(std::vector<uint32_t> const& _data, bool const& _sign) : data(_data), sign(_sign) {
  to_size();
}

big_integer::big_integer(int o) {
  data.push_back(uint32_t(o));
  sign = (o < 0);
  to_size();
}
...
uint32_t big_integer::value(size_t id) const {
  if (id < data.size()) {
    return data[id];
  }
...
big_integer operator + (big_integer const& a, big_integer const& b) {
  uint32_t carry = 0;
  std::vector<uint32_t> res(std::max(a.len(), b.len()) + 1);//extra qword
  for (size_t i = 0; i < res.size(); i++) {
    ull cur = ull(a.value(i)) + b.value(i) + carry;
    carry = cur >> 32;
    res[i] = uint32_t (cur); //default casting with %= 2^32
  }
...
big_integer operator * (big_integer const& a, big_integer const& b) {
  ...
  for (size_t i = 0; i < p.len(); i++) {
    ...
    for (size_t j = 0; j < q.len() || carry; j++) {
      ...
      res[at] = uint32_t (cur); //default casting with %= 2^32
      carry = cur >> 32;
    }
  }
...
uint32_t trial(uint32_t a, uint32_t b, uint32_t c) {
  ull res = a;
  res <<= 32;
  res += b;
  res /= c;
...
big_integer operator / (big_integer const& a, big_integer const& b) {
  ...
  uint32_t f = (ull(UINT32_MAX) + 1) / (ull(q.data.back()) + 1);
  big_integer bf = f;
  bf = bf.abs();
  p *= bf;
  q *= bf;
  ...
  size_t sz = n - m + 1;
  std::vector<uint32_t> res(sz);
  uint32_t divisor = q.data.back();
  std::vector<uint32_t> divident_pref(m + 1);
...
big_integer operator & (big_integer const& a, big_integer const& b) {
  std::vector<uint32_t> res(std::max(a.len(), b.len()));
